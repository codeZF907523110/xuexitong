...
<template>
  <div class="comment">
    <!-- 头部 -->
    <div class="c_head">
      <!-- <span v-if="$store.state.wenzhang">{{ $store.state.wenzhang.comment }}</span> -->
      <span>评论</span>
    </div>
    <!-- 主体部分 -->
    <div class="c_box">
      ...
      <!-- 评论列表 -->
      <div class="c_list" v-if="$store.state.wenzhang">
        <Clist
          v-for="(item, index) in $store.state.wenzhang.comment"
          :info="item"
          :index="index"
          @isreply="show"
...
<script>
import C_list from "components/wzpl/Commentlist.vue";
